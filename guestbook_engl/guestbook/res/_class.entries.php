...
class Entries{
	...
	function check_new_entry($new_entry){
		...
		// check web
		if(isset($new_entry['web']) && ""!=$new_entry['web'] && MAX_WEB_CHARACTERS<strlen($new_entry['web'])){
			$this->error[] = "###error_web_is_too_long### ".MAX_WEB_CHARACTERS;
		}
		
		
		// check message
		if(!isset($new_entry['web']) || ""==$new_entry['message']){
